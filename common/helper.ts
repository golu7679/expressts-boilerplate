...
export const isoFormattedDateTimeIncrease = (date: string, seconds: number) => {
  const newDate = new Date(date);
  return new Date(newDate.getTime() + seconds * 1000);
};

export const isNumber = (data: any, returnNumber = 1): number => {
  return data !== undefined && typeof +data === 'number' && !Number.isNaN(+data) ? +data : returnNumber ? returnNumber : 1;
};
...
export const optimizeUploadedImage = async (file: any) => {
  ...
  const quality = imageSize > 3 ? 40 : imageSize > 2 ? 60 : imageSize > 1 ? 70 : 80;
  if (fileNewName.endsWith('jpeg') || fileNewName.endsWith('jpg')) await sharp(file.path).rotate().resize().jpeg({ quality }).toFile(newFilePath);
  else await sharp(file.path).rotate().resize().png({ quality }).toFile(newFilePath);
...
export const generate8DigitCode = async () => {
...
export const generateSlug = (name: string): string => {
  ...
  const removedSpecialCharacter = lowerCase.replace(/[&\/\\#,+()$~%.'":*?<>{}]/g, '');
  const removedMultipleSpaces: string = removedSpecialCharacter.replace(/\s\s+/g, ' ');
